...
if (method == "insert"){
    ...
    editor.addEventListener("DOMSubtreeModified", (e)=>{
        if (e.target != editor && e.target.nodeName != "#text"){
            //console.log(e)
            //e.target.setAttribute("class", "paragraph")
        }
    })
    
    document.querySelector("#editor .paragraph").addEventListener("focusin", (e)=>{
        console.log("focus", e)
    })
}
// 크롬에서만 작동한다고 하며, 추후 ios 까지 확장되었을 때를 생각하면 이렇게 짜면 안될것 같음
...
// jquery 사용
// 부모와 자식이 둘 다 contentEditable 일 경우 이벤트 캡처링이 제대로 이루어지지 않는다고 함
else if (method == "jq_insert"){
